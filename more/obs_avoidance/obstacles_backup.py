...
class Obstacle():
    ...
    def __init__(self, pos, dimensions, obs_type, f_max, f_min=None, min_dist=1e-2, obs_name=None, eps_meters=None, e_max=1):
        ...
        self.rot_field_unlocked = True # If False, the panagou rotational field direction will be locked
        self.last_rot_field_dir = None
...
def ObstacleAvoidanceControllerGenerator(agent: Agent, obs_list, func_name=None):
    ...
    def obstacle_avoidance_control(x, t):
        ...
        def panagouRotationalFieldCalc(p_command, rj, obstacle):
            # Lets normalise the p vector
            if np.linalg.norm(p_command) != 0:
                p_command = p_command / np.linalg.norm(p_command)   # TODO: P vector scales the output. instead of having it constant, it could change with speed intent, but in a controlled manner
            # Update the p vector using locked field status
            if obstacle.rot_field_unlocked:
                # Update the direction in memory
                obstacle.last_rot_field_dir = p_command
                p = p_command
            else:
                # If the field is locked, we use the last direction
                p = obstacle.last_rot_field_dir if obstacle.last_rot_field_dir is not None else p_command
                

            r0 = obstacle.pos
            if obstacle.type == 'circle':
                r = rj - r0
                if np.linalg.norm(r) > obstacle.r and np.linalg.norm(r) < 1.5 * obstacle.r:
                    if obstacle.rot_field_unlocked:
                        obstacle.rot_field_unlocked = False  # Lock the direction of the field

                    if p.T @ r >= 0:
                        Fp = np.dot(p, r) * r - p * np.dot(r, r)
                    else:
                        Fp = - p * np.dot(r, r)

                else:
                    obstacle.rot_field_unlocked = True  # Unlock the direction of the field
                    Fp = np.zeros_like(r0, dtype=np.float64)
            elif obstacle.type == 'rectangle':
                if (abs(rj[0] - r0[0]) > obstacle.width/2 or abs(rj[1] - r0[1]) > obstacle.height/2) and (abs(rj[0] - r0[0]) < 2 * obstacle.width/2 and abs(rj[1] - r0[1]) < 2 * obstacle.height/2):
                    if obstacle.rot_field_unlocked:
                        obstacle.rot_field_unlocked = False  # Lock the direction of the field
                ...
                Fp = np.zeros_like(r0, dtype=np.float64)  # For walls, we don't calculate a force here

            # Debug print 
            # if Fp.all() != 0:
            #     print(agent.model.state)
            #     print(f"Obstacle: {obstacle.name_id} \t- p: {p} \t- rj: {rj} \t- r0: {r0} \t- Fp: {Fp}")
            return Fp

        # Iterate over all obstacles
        for obstacle in obs_list:
            fp = panagouRotationalFieldCalc(agent.model.state[6:8], x[:2], obstacle)
            ...
            # Append the panagou calculation to the already calculated potential field
