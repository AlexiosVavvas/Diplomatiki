...
# Ensure the repo root is on sys.path so src.ergodic_exploration.* imports work
...
# IMPORTANT: These bounds MUST match the agent's L_BOUNDS for correct reconstruction!

Kmax = 7
L1_min = -20
L1_max = 380  # Must match agent's --l_bounds
L2_min = -20
L2_max = 380  # Must match agent's --l_bounds
...
# Position inversion flag for NED convention (airplanes)
# When True, X/Y axes are transposed in the visualization
POS_INVERTED = False
...
def plotPhi(phi_rec_from_ck, phi_rec_from_agent, phi_rec_from_ros_ck, all_traj=None, grid_res=50, clip_to_min_max=False, ergodic_cost=None):
    ...
    # Apply position inversion (transpose) for NED convention if enabled
    display_Z_ros = Z_rec_from_ros_ck.T if POS_INVERTED else Z_rec_from_ros_ck

    ax3 = fig.add_subplot(133)
    if clip_to_min_max:
        im3 = ax3.imshow(display_Z_ros, extent=(base.L1_min, base.L1_max, base.L2_min, base.L2_max), 
                        origin='lower', cmap=cm.viridis, vmin=min_val, vmax=max_val)
    else:
        im3 = ax3.imshow(display_Z_ros, extent=(base.L1_min, base.L1_max, base.L2_min, base.L2_max), 
                        origin='lower', cmap=cm.viridis)
    ax3.set_title('Reconstructed from ROS Ck (ck_values_average_in_range)' + (' [pos inverted]' if POS_INVERTED else ''))
    ax3.set_xlabel('x1')
    ax3.set_ylabel('x2')
    ax3.set_aspect('auto')
...
def plot_ros_only(phi_rec_from_ros_ck, agent_id, grid_res=50, ergodic_cost=None):
    ...
    # Apply position inversion (transpose) for NED convention if enabled
    display_data = Z_rec_from_ros_ck.T if POS_INVERTED else Z_rec_from_ros_ck
        
    fig = plt.figure(figsize=(8, 6))
    ax = fig.add_subplot(111)

    im = ax.imshow(display_data, extent=(base.L1_min, base.L1_max, base.L2_min, base.L2_max), origin='lower', cmap='viridis')
    ax.set_title(f'Agent {agent_id} - ROS Ck (ck_values_average_in_range)' + (' [pos inverted]' if POS_INVERTED else ''))
    ax.set_xlabel('x1')
    ax.set_ylabel('x2')
    ax.set_aspect('equal')
...
class RealTimeVisualizer:
    ...
    def update_plot(self, frame):
        ...
        if self.ck_subscriber.latest_ck_values is None:
        ...
        # Use agent's own ck_values for visualization (not ck_values_average which is other agents' average)
        # ck_values = self.ck_subscriber.latest_ck_values
        ...
        try:
            ...
            # Apply position inversion (transpose) for NED convention if enabled
            display_data_3d = self.Z_rec_from_ros_ck.T if POS_INVERTED else self.Z_rec_from_ros_ck
            ...
            if USE_3D_PLOT:
                ...
                if USE_FIXED_COLOR_RANGE:
                    self.surf3 = self.ax3.plot_surface(self.X, self.Y, display_data_3d, 
                                                      cmap='viridis', vmin=self.vmin, vmax=self.vmax,
                                                      alpha=0.8, antialiased=True)
                    # Set z-axis limits to match color range for consistent visualization
                    self.ax3.set_zlim(self.vmin, self.vmax)
                ...
                # Apply position inversion (transpose) for NED convention if enabled
                display_data = self.Z_rec_from_ros_ck.T if POS_INVERTED else self.Z_rec_from_ros_ck
                self.im3.set_array(display_data)
                
                # Apply color scaling based on configuration
                if not USE_FIXED_COLOR_RANGE:
                    # Dynamic color range (original behavior)
                    self.im3.set_clim(vmin=display_data.min(), vmax=display_data.max())
    ...
    def stop(self):
        ...
        if hasattr(self, 'ani') and self.ani is not None:
            try:
                # First, pause the animation to prevent further callbacks
                if hasattr(self.ani, 'pause'):
                    self.ani.pause()
                ...
                if hasattr(self.ani, 'event_source') and self.ani.event_source is not None:
                    ...
                    # Also remove the timer to prevent any further callbacks
                    if hasattr(self.ani, '_fig') and self.ani._fig is not None:
                        try:
                            self.ani._fig.canvas.mpl_disconnect(self.ani._first_draw_id) if hasattr(self.ani, '_first_draw_id') else None
                        except:
                            pass
                ...
                # Silently handle other errors during shutdown
                pass
            finally:
                # Set ani to None early to prevent race conditions
                self.ani = None
        ...
        if hasattr(self, 'fig') and self.fig is not None:
            try:
                ...
                pass  # Silently handle figure close errors
            finally:
                self.fig = None
...
def main():
    ...
    parser.add_argument('--pos_inverted', action='store_true',
                       help='Invert X/Y axes for agents using NED convention (airplanes). When set, the heatmap axes will be transposed.')
    ...
    try:
        ...
        # Update position inversion flag
        global POS_INVERTED
        POS_INVERTED = args.pos_inverted
        if POS_INVERTED:
            print("Position inverted: X/Y axes will be transposed (NED convention)")
        ...
        if POS_INVERTED:
            print("Position: inverted (NED convention)")
        ...
        print("  --pos_inverted: Transpose X/Y axes for NED convention agents")
