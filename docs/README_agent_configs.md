...
### 4. Phi Section (Exploration Density)
- `type`: The type of phi function (`'uniform'` or `'gaussian_bumps'`)
- `bumps`: List of Gaussian bump configurations (only used when type is `'gaussian_bumps'`)
...
#### Phi Configuration (Exploration Density)
The `phi` section controls the exploration density function:

- `type: uniform` - Constant uniform coverage across the entire domain (default)
- `type: gaussian_bumps` - Gaussian bump-based coverage with configurable hotspots

For `gaussian_bumps`, additional parameters:
- `base_level`: Base exploration level added everywhere to ensure some exploration happens outside bump locations (default: 0.01)

Each bump is defined with:
- `x_pos_perc`: X position as percentage of L1 bounds (0.0 to 1.0)
- `y_pos_perc`: Y position as percentage of L2 bounds (0.0 to 1.0)  
- `height`: Amplitude/height of the bump
- `width_scale`: Width scaling factor (larger = sharper/narrower, smaller = wider/smoother)
...
# OR Gaussian bumps coverage
phi:
  type: gaussian_bumps
  base_level: 0.01   # Promotes exploration everywhere, not just at bumps
  bumps:
    - x_pos_perc: 0.7    # 70% along x-axis
      y_pos_perc: 0.3    # 30% along y-axis
      height: 5
      width_scale: 0.7
    - x_pos_perc: 0.3
      y_pos_perc: 0.7
